        ...
        if (elytraFly.replace.get()) {
            ...
            if (chestStack.getItem() == Items.ELYTRA) {
                if(Modules.get().get(AutoLog.class).isActive() && elytraFly.disconnectwhenlackelytra.get()){
                    ...
                    if(chestStack.getMaxDamage() - chestStack.getDamage()==elytraFly.lastelytraDurability.get()){
                        elytracount=leftelytracount();
                    }
                    if(elytracount<=1 && chestStack.getMaxDamage() - chestStack.getDamage()<=elytraFly.lastelytraDurability.get()){
                        mc.player.setPitch(-0.5F);
