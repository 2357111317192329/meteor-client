...
public class DamageUtils {
    ...
    public static float crystalDamage(LivingEntity target, Vec3d crystal, boolean predictMovement, BlockPos obsidianPos) {
        return overridingExplosionDamage(target, crystal, 6f, predictMovement, obsidianPos, Blocks.OBSIDIAN.getDefaultState());
    }
    ...
    public static float anchorDamage(LivingEntity target, Vec3d anchor) {
        return overridingExplosionDamage(target, anchor, 5f, false, BlockPos.ofFloored(anchor), Blocks.AIR.getDefaultState());
    }
    ...
    private static float pvpexplosionDamage(LivingEntity target, Vec3d explosionPos, float power, boolean predictMovement) {
        return explosionDamage(target, explosionPos, power, predictMovement, HIT_FACTORY);
    }
    public static float explosionDamage(LivingEntity target, Vec3d explosionPos, float power, boolean predictMovement) {
    ...
    public static float getAttackDamage(LivingEntity attacker, Entity target) {
        EntityAttributeInstance inst = attacker.getAttributeInstance(EntityAttributes.ATTACK_DAMAGE);
        float itemDamage = (float) ((inst != null) ? inst.getValue() : 0.0);
